...
test 'user can edit proxy endpoint components', (assert) ->
  ...
  after = ->
    ...
    andThen ->
      assert.equal currentURL(), '/apis/1/proxy-endpoints/1/components/1/edit'
      assert.equal server.db.proxyEndpoints[0].components[0].conditional, 'console.log("test");'
      done()
  ...
  andThen ->
    findWithAssert('[data-t="fields.conditional"] .ap-ace-editor')[0].aceEditor.getSession().setValue 'console.log("test");'
    click '[type=submit]'
