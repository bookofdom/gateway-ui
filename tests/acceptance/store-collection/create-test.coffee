...
test 'user can create new store collections', (assert) ->
  ...
  andThen ->
    ...
    assert.equal find('.ap-app-secondary .ap-app-secondary .ap-app-secondary-sidebar > .ap-list-nav li:not([data-t="actions.new"])').length, beforeCreateCount
  click '.ap-app-secondary .ap-app-secondary .ap-app-secondary-sidebar > .ap-list-nav li[data-t="actions.new"] a'
  fillIn '[name=name]', 'New Collection'
  click '[type=submit]'
  andThen ->
    assert.equal server.db.storeCollections.length, beforeCreateCount + 1
    assert.equal find('.ap-app-secondary .ap-app-secondary .ap-app-secondary-sidebar > .ap-list-nav li:not([data-t="actions.new"])').length, beforeCreateCount + 1
