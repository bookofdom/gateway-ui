...
moduleForModel 'proxy-endpoint-component-call', 'Unit | Serializer | proxy endpoint component call',
...
test 'it normalizes records', (assert) ->
  @store().findAll('api').then (apis) ->
    ...
    api.get('proxy_endpoints').then (proxy_endpoints) ->
      ...
      proxy_endpoint_component_calls = proxy_endpoint_component.get('calls')
      assert.equal proxy_endpoint_component_calls.get('length'), 2
