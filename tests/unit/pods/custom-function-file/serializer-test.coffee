...
moduleForModel 'custom-function-file', 'Unit | Serializer | custom function file',
  needs: [
    'serializer:api'
    'serializer:custom-function'
    'serializer:custom-function-file'
    'model:api'
  ...
  beforeEach: ->
    @server = new Pretender ->
      ...
      @get '/apis/1/custom_functions/1/files', -> [
        200
        {'Content-Type': 'application/json'}
...
test 'it normalizes records', (assert) ->
  @store().findAll('api').then (apis) ->
    ...
    api.get('custom_functions').then (customFunctions) ->
      ...
      customFunction.get('files').then (files) ->
        assert.equal files.get('length'), 1
