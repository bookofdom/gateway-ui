...
moduleForModel 'push-channel-message', 'Unit | Serializer | push channel message',
  needs: [
    ...
    'serializer:push-channel-message'
    'model:api'
  ...
  beforeEach: ->
    @server = new Pretender ->
      ...
      @get '/push-channel-messages', -> [
        200
        {'Content-Type': 'application/json'}
        JSON.stringify push_channel_messages: [
          id: 1
          push_channel_id: 1
          stamp: 1463415460
          data:
...
test 'it normalizes records', (assert) ->
  store = @store()
  store.findAll('push-channel-message').then (messages) ->
