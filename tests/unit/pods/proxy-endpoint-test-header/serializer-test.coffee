...
moduleForModel 'proxy-endpoint-test-header', 'Unit | Serializer | proxy endpoint test header',
  needs: [
    'serializer:api'
      ...
      @get '/apis/1/proxy_endpoints', -> [
        ...
        JSON.stringify proxy_endpoints: [
          tests: [
            ...
            pairs: [
              id: 1 # proxy-endpoint-test-header
              type: 'header'
              key: 'key1'
              value: 'value1'
            ,
              id: 2 # proxy-endpoint-test-header
              type: 'header'
              key: 'key2'
              value: 'value2'
            ]
...
test 'it normalizes records', (assert) ->
  @store().findAll('api').then (apis) ->
    ...
    api.get('proxy_endpoints').then (proxy_endpoints) ->
      ...
      proxy_endpoint_test_headers = proxy_endpoint_test.get('headers')
      assert.equal proxy_endpoint_test_headers.get('length'), 2
